  ...
  loadSignature({editor, bufferPosition}) {
    ...
    .then(data => {
      if (data) {
        ...
        element.classList.toggle('signature-above-completions', atom.config.get('kite.signatureAboveCompletions'));
    ...
    .then(() => {
      console.log(this.getSuggestionsListElement().maxVisibleSuggestions);
    })
    .catch(err => {
      console.error(err);
